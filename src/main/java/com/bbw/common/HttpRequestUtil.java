...
public class HttpRequestUtil {
	...
	public static String getRequestBody(HttpServletRequest request) {
		...
		try {
			...
			requestParams = stringBuffer.toString();
		} catch (Exception e) {
	...
	public static String getRequestParams(HttpServletRequest request) {
		...
		// 去掉最后一个空格
		String qureyParam = queryString.toString().substring(0, queryString.length() - 1);
		return qureyParam;
	}

	/**
	 * 获取HTTP请求的参数，名字相同的数组只取第一个
	 * @param request
	 * @return
	 */
	public static Map<String, String> getUniqueParams(HttpServletRequest request) {
	 ...
	 * <pre>
	 * 获取唯一值的参数，并添加客户端ip地址。
	 * 程序员记录调试信息时候使用，不可用于支付回调参数获取。
	 * </pre>
	 * @param request
	 * @return
	 */
	public static Map<String, String> getUniqueParamsAddIp(HttpServletRequest request) {
		String ip = IpUtil.getIpAddr(request);
		Map<String, String> params = HttpRequestUtil.getUniqueParams(request);
		params.put("IP", ip);
		return params;
	}
	...
	public static String getLogString(HttpServletRequest request) {
		String getUrl = request.getRequestURI() + "?" + HttpRequestUtil.getRequestParams(request);
		return getUrl;
	}
