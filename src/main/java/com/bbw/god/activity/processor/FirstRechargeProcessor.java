 ...
 **/
@Service
public class FirstRechargeProcessor extends AbstractActivityProcessor {
    @Autowired
    private MailService mailService;
    private static final int FIRST_RECHARGE_198 = 63;
    private static final List<Integer> FIRST_RECHARGE = Arrays.asList(60, 61, 62, 63);

    public FirstRechargeProcessor() {
        this.activityTypeList = Arrays.asList(ActivityEnum.FIRST_R);
    }
    ...
    public void rechargeItem(UserReceipt param) {
        ...
        UserMail mail = UserMail.newAwardMail(title, content, param.getGameUserId(), Arrays.asList(Award.instance(TreasureEnum.JXQ.getValue(), AwardEnum.FB, 8)));
        mailService.send(mail);
    }

    private CfgActivityEntity getRechargeActivity() {
        return ActivityTool.getActivityByType(ActivityEnum.FIRST_R);
    }
    ...
    public boolean isFirst(GameUser gu) {
        IActivity a = this.activityService.getActivity(gu.getServerId(), ActivityEnum.FIRST_R);
