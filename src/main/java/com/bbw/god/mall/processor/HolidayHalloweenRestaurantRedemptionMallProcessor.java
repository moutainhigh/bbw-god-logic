 ...
 */
@Service
public class HolidayHalloweenRestaurantRedemptionMallProcessor extends AbstractMallProcessor {
    @Autowired
    private HolidayHalloweenRestaurantRedemptionMallProcessor() {
        this.mallType = MallEnum.HALLOWEEN_HALLOWEEN_RESTAURANT_REDEEM;
    }
    ...
    public RDMallList getGoods(long guId) {
        ...
        List<CfgMallEntity> list = MallTool.getMallConfig().getHalloweenRestaurantRedemptionMalls().stream()
    ...
    public void deliver(long guId, CfgMallEntity mall, int buyNum, RDCommon rd) {
        ...
        if (AwardEnum.fromValue(mall.getItem()) == AwardEnum.TQ) {
            ResEventPublisher.pubCopperAddEvent(guId, num, WayEnum.HALLOWEEN_HALLOWEEN_RESTAURANT_REDEEM, rd);
        } else {
            TreasureEventPublisher.pubTAddEvent(guId, mall.getGoodsId(), num, WayEnum.HALLOWEEN_HALLOWEEN_RESTAURANT_REDEEM, rd);
        }
    }
