...
/**
 * @author suchaobin
 * @description 任务id%1000=211的任务对应的service
 * @date 2020/11/25 15:32
 **/
@Service
public class DailyTaskService211 extends ResourceDailyTaskService {
    /**
    ...
    public List<Integer> getMyTaskIds() {
        return Arrays.asList(22211, 23211);
    }
    ...
    public int doGetProgress(long uid, int level, int taskId, UserTaskInfo info) {
        SpecialStatistic statistic = specialStatisticService.fromRedis(uid, StatisticTypeEnum.GAIN, DateUtil.getTodayInt());
        Map<WayEnum, Integer> todayMap = statistic.getTodayMap();
        Integer val = todayMap.get(WayEnum.YSG);
