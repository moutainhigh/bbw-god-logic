...
/**
 * @author suchaobin
 * @description 任务id%1000=110的任务对应的service
 * @date 2020/11/24 21:35
 **/
@Service
public class DailyTaskService110 extends ResourceDailyTaskService {
    /**
    ...
    public List<Integer> getMyTaskIds() {
        return Arrays.asList(21110, 22110, 23110, 24110, 25110);
    }
    ...
    public int doGetProgress(long uid, int level, int taskId, UserTaskInfo info) {
        GoldStatistic statistic = goldStatisticService.fromRedis(uid, StatisticTypeEnum.GAIN, DateUtil.getTodayInt());
        Map<WayEnum, Integer> todayMap = statistic.getTodayMap();
        Integer val = todayMap.get(WayEnum.FD);
