 ...
 */
@Service
public class UserNewYearAndChristTaskService {
    @Autowired
    ...
    public List<UserTimeLimitTask> getAllTasks(long uid) {
        return userTimeLimitTaskService.getAllTasks(uid, TaskGroupEnum.NEW_YEAR_AND_CHRISTMAS_TASK);
    }
    ...
    public List<UserTimeLimitTask> getTasks(long uid) {
        return userTimeLimitTaskService.getTasks(uid, TaskGroupEnum.NEW_YEAR_AND_CHRISTMAS_TASK);
    }
    ...
    public UserTimeLimitTask makeUserTaskInstance(long uid, CfgTaskEntity task) {
        ...
        if (TaskTypeEnum.TIME_LIMIT_DISPATCH_TASK.getValue() == task.getType()) {
            instance = UserTimeLimitTask.instance(uid, TaskGroupEnum.NEW_YEAR_AND_CHRISTMAS_TASK, task, getRandomSkills(task), null);
        } else {
            instance = UserTimeLimitTask.instance(uid, TaskGroupEnum.NEW_YEAR_AND_CHRISTMAS_TASK, task);
        }
        return instance;
    }
    ...
    private List<Integer> getRandomSkills(CfgTaskEntity task) {
        String cacheKey = "newYearAndChristmasTask" + task.getId();
        List<Integer> fromCache = GameDataTimeLimitCacheUtil.getFromCache(cacheKey, List.class);
        if (null != fromCache) {
          return fromCache;
        }
        fromCache = TimeLimitTaskTool.getExtraRandomSkills(TaskGroupEnum.NEW_YEAR_AND_CHRISTMAS_TASK, task);
