...
/**
 * @author suchaobin
 * @description 任务id%1000=18的任务对应的service
 * @date 2020/11/24 16:21
 **/
@Service
public class DailyTaskService18 extends ResourceDailyTaskService {
    /**
    ...
    public List<Integer> getMyTaskIds() {
        return Arrays.asList(21018, 22018, 23018, 24018, 25018);
    }
    ...
    public int doGetProgress(long uid, int level, int taskId, UserTaskInfo info) {
        GoldStatistic statistic = goldStatisticService.fromRedis(uid, StatisticTypeEnum.CONSUME, DateUtil.getTodayInt());
        Map<WayEnum, Integer> todayMap = statistic.getTodayMap();
        Integer val = todayMap.get(WayEnum.MALL_BUY);
