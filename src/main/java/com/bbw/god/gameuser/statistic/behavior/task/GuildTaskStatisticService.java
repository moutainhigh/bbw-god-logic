...
/**
 * @author suchaobin
 * @description 行会任务统计service
 * @date 2020/11/25 16:31
 */
@Service
public class GuildTaskStatisticService extends BehaviorStatisticService {
    ...
    public <T extends BaseStatistic> void toRedis(long uid, T statistic) {
        if (!(statistic instanceof GuildTaskStatistic)) {
            throw new CoderException("参数类型错误！");
        }
        GuildTaskStatistic guildTaskStatistic = (GuildTaskStatistic) statistic;
        Integer date = guildTaskStatistic.getDate();
        Integer today = guildTaskStatistic.getToday();
        Integer total = guildTaskStatistic.getTotal();
    ...
    public <T extends BaseStatistic> T buildStatisticFromPreparedData(long uid, StatisticTypeEnum typeEnum, int date, Map<String, Map<String, Object>> preparedData) {
        ...
        return (T) getGuildTaskStatistic(date, redisMap);
    }
    ...
    @Override
    @SuppressWarnings("unchecked")
    public GuildTaskStatistic fromRedis(long uid, StatisticTypeEnum typeEnum, int date) {
        String redisKey = getKey(uid, typeEnum);
        Map<String, Integer> redisMap = redisHashUtil.get(redisKey);
        return getGuildTaskStatistic(date, redisMap);
    }

    private GuildTaskStatistic getGuildTaskStatistic(int date, Map<String, Integer> redisMap) {
        ...
        return new GuildTaskStatistic(today, total, date);
    }
