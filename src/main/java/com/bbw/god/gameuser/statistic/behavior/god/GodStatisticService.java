...
/**
 * @author suchaobin
 * @description 遇到神仙统计service
 * @date 2020/4/23 11:22
 */
@Service
public class GodStatisticService extends BehaviorStatisticService {
    /**
     * 获取当前行为类型
     *
     * @return 当前行为类型
    ...
    public <T extends BaseStatistic> void toRedis(long uid, T statistic) {
        if (!(statistic instanceof GodStatistic)) {
            throw new CoderException("参数类型错误！");
        }
        GodStatistic godStatistic = (GodStatistic) statistic;
        Integer date = godStatistic.getDate();
        Map<String, Integer> map = new HashMap<>();
        map.put(date + UNDERLINE + NUM, godStatistic.getToday());
        map.put(TOTAL, godStatistic.getTotal());
    ...
    public <T extends BaseStatistic> T buildStatisticFromPreparedData(long uid, StatisticTypeEnum typeEnum, int date, Map<String, Map<String, Object>> preparedData) {
        ...
        return (T) getGodStatistic(date, redisMap);
    }
    ...
    @Override
    @SuppressWarnings("unchecked")
    public GodStatistic fromRedis(long uid, StatisticTypeEnum typeEnum, int date) {
        String key = getKey(uid, typeEnum);
        Map<String, Integer> redisMap = redisHashUtil.get(key);
        return getGodStatistic(date, redisMap);
    }

    private GodStatistic getGodStatistic(int date, Map<String, Integer> redisMap) {
        ...
        return new GodStatistic(today, total, date);
    }
