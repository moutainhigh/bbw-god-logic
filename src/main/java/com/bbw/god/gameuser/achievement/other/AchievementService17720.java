...
/**
 * 卡牌类成就17720service 控宝大成III
 *
 * @author: huanghb
 * @date: 2022/5/4 16:32
 */
@Service
public class AchievementService17720 extends BaseAchievementService {
	@Autowired
	...
	public int getMyProgress(long uid, UserAchievementInfo info) {
		...
		List<UserCardXianJue> xianJues = userCardXianJueService.getUserCardXianJuesByXianJueType(uid, XianJueTypeEnum.KONG_BAO_SHU.getValue());
		//获得进度
		return (int) xianJues.stream().filter(tmp -> CfgXianJueTool.getMaxQuality() == tmp.getQuality()).count();
	}
}
