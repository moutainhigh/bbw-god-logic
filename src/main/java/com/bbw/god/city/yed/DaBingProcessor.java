 ...
 **/
@Service
public class DaBingProcessor extends ExtraYeDEventProcessor implements YeDEventExtraOperation {
	@Autowired
	...
	@Override
	public int getMyId() {
		return YdEventEnum.DA_BING.getValue();
	}
	...
	public void effect(GameUser gameUser, RDArriveYeD rdArriveYeD, RDAdvance rd) {
		...
		long deductCopper = 500 * gameUser.getLevel();
		...
		CityEventPublisher.pubYeDTrigger(bep, EPYeDTrigger.fromLoss(YdEventEnum.DA_BING, deductCopper));
	}
	...
	public void extraOperation(GameUser gu, Integer num, RDArriveYeD arriveYeD, RDAdvance rd) {
		...
		RDFightsInfo fightsInfo = yeDFightProcessor.buildRDFightsInfo(gu.getId(),cityLv,cityId,YdEventEnum.DA_BING);
		fightsInfo.setNickname("妲己卫兵");
		fightsInfo.setHead(TreasureEnum.HEAD_DJ.getValue());
