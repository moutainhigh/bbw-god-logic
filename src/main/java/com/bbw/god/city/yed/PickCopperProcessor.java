...
/**
 * @author suchaobin
 * @description 捡到铜钱事件处理器
 * @date 2020/6/1 11:03
 **/
@Service
public class PickCopperProcessor extends BaseYeDEventProcessor {
	/**
	 * 获取当前野地事件id
	 */
	@Override
	public int getMyId() {
		return YdEventEnum.PICK_COPPER.getValue();
	}
	...
	public void effect(GameUser gameUser, RDArriveYeD rdArriveYeD, RDAdvance rd) {
		int addedCopper = 500 * gameUser.getLevel();
		...
		CityEventPublisher.pubYeDTrigger(bep, EPYeDTrigger.fromIncome(YdEventEnum.PICK_COPPER, addedCopper));
	}
}
