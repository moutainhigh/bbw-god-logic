...
/**
 * @author suchaobin
 * @description 幸运号码事件处理器
 * @date 2020/6/1 11:03
 **/
@Service
public class LuckyNumProcessor extends BaseYeDEventProcessor {
	/**
	 * 获取当前野地事件id
	 */
	@Override
	public int getMyId() {
		return YdEventEnum.LUCKY_NUM.getValue();
	}
	...
	public void effect(GameUser gameUser, RDArriveYeD rdArriveYeD, RDAdvance rd) {
		int addedCopper = PowerRandom.getRandomBetween(10, 60) * 360;
		...
		CityEventPublisher.pubYeDTrigger(bep, EPYeDTrigger.fromIncome(YdEventEnum.LUCKY_NUM, addedCopper));
	}
}
