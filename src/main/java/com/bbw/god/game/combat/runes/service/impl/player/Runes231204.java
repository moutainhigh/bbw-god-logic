...
/**
 * 神剑符图	3阶	己方卡牌攻击时，有30%概率（可升级）使该卡牌在本回合获得【神剑】。	每级额外+7%概率
 *
 * @author: suhq
 * @date: 2021/11/12 5:33 下午
 */
@Service
public class Runes231204 implements IRoundStageRunes {
    @Autowired
    private RuneAddSkillService runeAddSkillService;

    @Override
    public int getRunesId() {
        return RunesEnum.SHEN_JIAN_PLAYER.getRunesId();
    }
    ...
    public Action doRoundRunes(CombatRunesParam param) {
        ...
        for (BattleCard card : player.getPlayingCards()) {
            ...
            if (combatBuff.ifToPerform(30, 7)) {
                runeAddSkillService.addSkillTOCard(getRunesId(), card, CombatSkillEnum.SHENJ);
            }
        }
        return action;
    }
