...
/**
 * 毒盾符图	1阶	战斗开始时，有30%概率（可升级）发动，己方卡牌受到的毒系技能伤害减少30%。	每级额外+7%概率
 *
 * @author: suhq
 * @date: 2021/11/12 3:04 下午
 */
@Service
public class Runes231003 extends PlayerStatusRune {
    @Autowired
    private DuSeriesService duSeriesService;

    @Override
    public int getRunesId() {
        return RunesEnum.DU_DUN_PLAYER.getRunesId();
    }

    @Override
    Action handleStatus(CombatRunesParam param) {
        return duSeriesService.deductInjure(getRunesId(), param, 0.3);
    }
}
